...
const filterReport = async () => {
  try {
    await reportPinia.fetchRevenue({
      start_date: new Date(report.value.start_date),
      end_date: new Date(report.value.end_date)
    })
  } catch (error) {
    console.log(error)
  }
...
const filteredRoles = computed(() => {
  return physiotherapist.value.filter((item) => item.role.name == 'Physiotherapist')
})
...
<template>
  <div class="m-4">
    ...
    <Toolbar class="bg-gray-900">
      ...
      <template #end>
        <button
          class="bg-primary rounded text-white p-2 md:mt-8 align-items-center"
          @click="filterCompareReport"
        >
          Apply Filter
        </button>
      </template>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-2">
      <cardDashboard description="Revenue" :number="allRevenue.revenue" />
      <cardDashboard description="Patients" :number="allPatients.length" />
      <cardDashboard description="Revenue" :number="physiotherapist.length" />
    </div>
    ...
    <DataTable
      class="h-screen"
      :value="allCompareRevenue.revenue_data"
      scrollable
      scrollHeight="600px"
      tableStyle="min-width: 50rem"
    >
    ...
    <Dialog v-model:visible="visible" modal header="Work statics" :style="{ width: '25rem' }">
      <div class="flex flex-col md:flex-row">
        ...
        <div class="">
          ...
          >
          <input
            v-model="phsioDate.end_date"
            id="name"
            type="date"
            class="w-full px-3 py-2 dark:text-black border border-gray-300 rounded focus:outline-none focus:border-primary"
            @change="filterPhysioReport"
            placeholder=""
          />
        </div>
      </div>
      ...
      >
        <Column header="Patient">
          <template #body="slotProps">
            <p class="capitalize">
              {{ slotProps.data.patient.first_name + ' ' + slotProps.data.patient.last_name }}
            </p>
          </template>
        </Column>
      ...
      <div class="flex justify-center gap-3 my-2">
        <Button
          class="bg-gray-200 hover:bg-primary text-black hover:text-white font-bold py-2 px-4 rounded"
          @click="visible = false"
        >
          Cancel
        </Button>
